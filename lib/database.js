var mongoose = require('mongoose')
  ...
  , Maxnode = require('../models/maxnode')
...
//Gets all Masternodes
  ...
  // Find A Maxnode by txid and offset
  find_maxnode: function (txhash, outidx, cb) {
    Maxnode.findOne({ txhash: txhash, outidx: outidx }, function (err, maxnode) {
      if (err) {
        return cb(null);
      } else {
        if (maxnode) {
  ...
  // Creates a New Maxnode
  create_maxnode: function (params, cb) {
    var newMaxnode = new Maxnode(params);
    newMaxnode.save(function (err) {
      if (err) {
        console.log(err);
        return cb();
      } else {
  ...
  // Update A Maxnode by txid and offset
  update_maxnode: function (params, cb) {
    if (!params.txhash == null || !params.outidx == null) {
      console.log("Maxnode Update - TX or Outidx is missing");
      console.log(params.txhash);
      console.log(params.outidx);
      return cb();
    }
    Maxnode.update({ txhash: params.txhash, outidx: params.outidx }, {
      rank: params.rank,
      network: params.network,
      txhash: params.txhash,
      outidx: params.outidx,
  ...
  // Removes Maxnode which are older than on day
  remove_old_maxnodes: function (cb)
  {
    Maxnode.remove({lastseen:{$lte:(Math.floor(Date.now() / 1000) - 86400)}}, function (err) {
      if (err) {
        console.log(err);
        return cb();
      } else {
...
//Gets all Maxnodes
  get_maxnodes: function (cb) {
    Maxnode.find({}, function (err, maxnode) {
      if (err) {
        return cb([]);
      } else {
        return cb(maxnode);
  ...
  update_cronjob_run: function (coin, param, cb) {
    ...
    if (param.list_maxnode_update) {
      Stats.update({ coin: coin }, {
        list_maxnode_update: param.list_maxnode_update
      }, function () {
        return cb(true);
      });
    }
